...
def test(model, test_loader, device, results_dir):
    ...
    with torch.no_grad():
        ...
        for data_dict in tqdm(test_loader):
            ...
            threshold_list.append(uncertainty.mean())
        ...
        for data_dict in tqdm(test_loader):
            ...
            mask_norm_rgb = ((pred_norm[0, :, :, :] + 1) * 0.5) * 255
            mask = uncertainty > threshold
            mask_norm_rgb[mask] = 0

            mask_norm_rgb = np.clip(mask_norm_rgb, a_min=0, a_max=255)
            mask_norm_rgb = mask_norm_rgb.astype(np.uint8)
            target_path = '%s/%s_filter.png' % (results_dir, img_name)
            plt.imsave(target_path, mask_norm_rgb)

            # 5. sampling weight
            weight = uncertainty
            weight[uncertainty < threshold] = 0
            valid = (weight > 1e-4).sum()

            print('valid:', valid / 640 / 480)
            print('mean:', weight.sum() / valid)
            print('max:', weight.max())

            target_path = '%s/%s_weight.npy' % (results_dir, img_name)
            np.save(target_path, weight)

            # 6. mask
            normal_mask = np.zeros_like(uncertainty).astype(np.float32)
            normal_mask[uncertainty <= threshold] = 1.0
            target_path = '%s/%s_mask.npy' % (results_dir, img_name)
            np.save(target_path, normal_mask)
